...
def build_staker_rewards(
    ...
    # Normalize
    active_now = set((t.get("address") if isinstance(t, dict) else t).lower()
                     for t in (active.get("tokens") or []) if t)
    ...
    dec_map = { (t.get("address") if isinstance(t,dict) else t).lower(): int(t.get("decimals", 18)) if isinstance(t,dict) else 18
                for t in (active.get("tokens") or []) }
